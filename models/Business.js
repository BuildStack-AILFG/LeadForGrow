...
import { withNewBusinessMessageDefaults } from '@/lib/crm/messageToggles';
...
const IntegrationCredentialsSchema = new mongoose.Schema({
  ...
  instagram: {
    ...
    lastVerified: { type: Date },
    // Set when the owner disconnects Instagram. While set, NO Instagram API call is made for this business
    // (getInstagramToken returns nothing, so the facebookAds fallback token can't be used either). Cleared on reconnect.
    disconnectedAt: { type: Date },
    webhookStatus: { type: String, enum: ['active', 'pending', 'error'], default: 'pending' },
    // Channel-level AI switch. When true, this channel's auto-reply DMs are
    // AI-generated by default (a rule can still force static/AI via replyMode).
    aiReplyEnabled: { type: Boolean, default: false },
    // Which comments become leads: 'matched' (default — only comments that trigger
    // an automation) or 'all'. NO schema default so an unset value means 'matched'
    // via lib/automation/commentMatch.js#commentLeadMode.
    commentLeadMode: { type: String, enum: ['matched', 'all'] },
    // Keyword → auto-response rules for comments on posts/reels. When a comment
    // matches a rule's keyword, we post a public reply and/or send the
    // commenter a DM automatically. See lib/instagram/commentAutomation.js.
    commentAutomations: [{
      id: { type: String },
      name: { type: String },                                   // optional label
      keywords: [{ type: String }],                             // any-of match
      matchType: { type: String, enum: ['contains', 'exact'], default: 'contains' },
      // Post scope: 'all' | 'specific' | 'next'. Deliberately NO schema default —
      // rules saved before scopes existed derive it (mediaId set ⇒ specific). See
      // lib/automation/postScope.js.
      scope: { type: String, enum: ['all', 'specific', 'next'] },
      mediaId: { type: String },                                // chosen/attached post; '' = none yet
      mediaThumb: { type: String },                             // preview shown in the settings UI
      mediaCaption: { type: String },
      mediaPermalink: { type: String },
      appliesFrom: { type: Date },                              // 'next' scope: only posts published after this
      publicReply: { type: String },                            // reply under the comment
      dmMessage: { type: String },                              // DM to the commenter
      // 'auto' = follow the channel's aiReplyEnabled switch. 'static' = always
      // send dmMessage (with {spintax}/||| variation). 'ai' = always generate a
      // context-aware DM per commenter via the LLM. See lib/ai/autoReply.js.
      replyMode: { type: String, enum: ['auto', 'static', 'ai'], default: 'auto' },
      aiInstruction: { type: String },                          // optional persona/guidance for AI mode
      enabled: { type: Boolean, default: true },
  ...
  // Facebook Page — Messenger DMs + post-comment automation. Kept as its own
  // block (parallel to `instagram`) so a business can enable Facebook on its own
  // without touching Instagram/WhatsApp/lead-ads. `facebookAds` above stays the
  // lead-ads integration; this is the messaging/comments channel.
  facebook: {
    enabled: { type: Boolean, default: false },
    ...
    messengerAutoReply: { type: Boolean, default: false },      // reserved for a future SLA-style auto-reply
    aiReplyEnabled: { type: Boolean, default: false },          // channel-level AI switch (see instagram.aiReplyEnabled)
    commentLeadMode: { type: String, enum: ['matched', 'all'] }, // see instagram.commentLeadMode
    // Same shape as instagram.commentAutomations — keyword rules for comments
    // on the Page's posts. See lib/facebook/commentAutomation.js.
    commentAutomations: [{
      id: { type: String },
      name: { type: String },
      keywords: [{ type: String }],
      matchType: { type: String, enum: ['contains', 'exact'], default: 'contains' },
      scope: { type: String, enum: ['all', 'specific', 'next'] }, // see instagram.commentAutomations
      mediaId: { type: String },                                // chosen/attached post_id; '' = none yet
      mediaThumb: { type: String },
      mediaCaption: { type: String },
      mediaPermalink: { type: String },
      appliesFrom: { type: Date },
      publicReply: { type: String },
      dmMessage: { type: String },                              // private reply to the commenter
      replyMode: { type: String, enum: ['auto', 'static', 'ai'], default: 'auto' },
...
BusinessSchema.index({ 'integrationCredentials.instagram.pageId': 1 }, { sparse: true });
BusinessSchema.index({ 'integrationCredentials.facebook.pageId': 1 }, { sparse: true });
...
// A NEW business must not message its customers on its own until the owner turns a message on. Existing documents are
// never touched by this hook (isNew only), and for them an unset toggle keeps meaning "on".
BusinessSchema.pre('validate', function () {
  if (!this.isNew) return;
  if (!this.settings) this.settings = {};
  this.set('settings.crm', withNewBusinessMessageDefaults(this.settings.crm));
});
