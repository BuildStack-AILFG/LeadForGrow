...
import { pickTaskTypeForLead } from '@/lib/crm/taskType';
  ...
  try {
    switch (node.type) {
      case 'send_whatsapp': {
        ...
        const result = await sendAutoWhatsApp(lead, business, msg, data.templateName, null, 'en', null, null, null, { origin: 'sequence' });
      ...
      case 'create_task': {
        ...
        await Task.create({
          ...
          type: data.taskType || pickTaskTypeForLead('call', lead),
        });
        log.status = 'success';
      ...
      case 'ai_whatsapp_reply': {
        ...
        if (!testMode) {
          const result = await sendAutoWhatsApp(lead, business, aiMsg, null, null, 'en', null, null, null, { origin: 'sequence' });
          log.status = result.success ? 'success' : 'failed';
      ...
      case 'ai_followup_timing': {
        ...
        if (node.type === 'ai_email') {
          ...
          await sendAutoWhatsApp(lead, business, result.reply, null, null, 'en', null, null, null, { origin: 'sequence' });
        }
        log.status = 'success';
...
// Routing rules live in ./edges (shared with the builder canvas and unit tested). A condition's "Yes" edge
// used to be skipped here because the label was compared with 'true' before the yes/no mapping.
export { getOutgoingEdges } from './edges';
