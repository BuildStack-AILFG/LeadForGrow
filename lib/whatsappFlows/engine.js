...
import { sendInstagramMessage, sendInstagramMedia } from '@/lib/social/guardedSend';
import { sendMessengerMessage, sendMessengerMedia } from '@/lib/social/guardedSend';
import { fetchExternal } from '@/lib/fetchExternal';
import { renderTemplate } from '@/lib/whatsappFlows/constants';

const MAX_STEPS = 40;

/** Instagram recipient id for a lead — set on lead.metadata by the IG handler. */
function igRecipientId(lead) {
  const m = lead?.metadata;
  if (!m) return null;
  return (typeof m.get === 'function' ? m.get('instagramId') : m.instagramId) || null;
}

/** Facebook Messenger recipient (PSID) — set on lead.metadata by the FB handler. */
function fbRecipientId(lead) {
  const m = lead?.metadata;
  if (!m) return null;
  return (typeof m.get === 'function' ? m.get('facebookId') : m.facebookId) || null;
}
...
export async function startFlowExecution({
  ...
  return continueExecution(execution._id, { business, lead, nodes, edges, channel: flow.channel || 'whatsapp', inboundText: variables.last_reply });
}
...
async function continueExecution(executionId, ctx) {
  ...
  const { business, lead, nodes, edges, channel = 'whatsapp' } = ctx;
  let steps = 0;
...
async function executeNode(node, ctx) {
  ...
  switch (type) {
    case 'action_send_text': {
      ...
      if (!isTest && lead && business) {
        if (ctx.channel === 'instagram') {
          const rid = igRecipientId(lead);
          if (!rid) throw new Error('No Instagram recipient id on lead');
          const result = await sendInstagramMessage(business, rid, text);
          if (!result.success) throw new Error(result.error || 'Failed to send Instagram DM');
        } else if (ctx.channel === 'facebook') {
          const rid = fbRecipientId(lead);
          if (!rid) throw new Error('No Facebook recipient id on lead');
          const result = await sendMessengerMessage(business, rid, text);
          if (!result.success) throw new Error(result.error || 'Failed to send Messenger DM');
        } else {
    ...
    case 'action_send_template': {
      ...
      if (!isTest && lead && business) {
        if (ctx.channel === 'instagram') {
          // IG has no WhatsApp-style templates — send the rendered text as a DM.
          const rid = igRecipientId(lead);
          if (!rid) throw new Error('No Instagram recipient id on lead');
          const result = await sendInstagramMessage(business, rid, text);
          if (!result.success) throw new Error(result.error || 'Failed to send Instagram DM');
        } else if (ctx.channel === 'facebook') {
          // Messenger has no templates — send the rendered text as a DM.
          const rid = fbRecipientId(lead);
          if (!rid) throw new Error('No Facebook recipient id on lead');
          const result = await sendMessengerMessage(business, rid, text);
          if (!result.success) throw new Error(result.error || 'Failed to send Messenger DM');
    ...
    case 'action_send_audio': {
      ...
      if (!isTest && lead && business) {
        if (ctx.channel === 'instagram') {
          const rid = igRecipientId(lead);
          if (!rid) throw new Error('No Instagram recipient id on lead');
          if ((mediaType === 'image' || mediaType === 'video') && data.mediaUrl) {
            const result = await sendInstagramMedia(business, rid, { mediaUrl: data.mediaUrl, messageType: mediaType });
            if (!result.success) throw new Error(result.error || 'Instagram media send failed');
          } else {
            // IG DM doesn't take documents/audio — send the link as a DM.
            const body = [caption, data.mediaUrl].filter(Boolean).join('\n');
            const result = await sendInstagramMessage(business, rid, body || 'Attachment');
            if (!result.success) throw new Error(result.error || 'Instagram DM send failed');
          }
        } else if (ctx.channel === 'facebook') {
          const rid = fbRecipientId(lead);
          if (!rid) throw new Error('No Facebook recipient id on lead');
          if ((mediaType === 'image' || mediaType === 'video') && data.mediaUrl) {
            const result = await sendMessengerMedia(business, rid, { mediaUrl: data.mediaUrl, messageType: mediaType });
            if (!result.success) throw new Error(result.error || 'Messenger media send failed');
          } else {
            // Messenger DM here sends documents/audio as a link.
            const body = [caption, data.mediaUrl].filter(Boolean).join('\n');
            const result = await sendMessengerMessage(business, rid, body || 'Attachment');
            if (!result.success) throw new Error(result.error || 'Messenger DM send failed');
          }
        } else {
    ...
    case 'action_send_buttons': {
      ...
      if (!isTest && lead && business) {
        if (ctx.channel === 'instagram') {
          // IG DM has no interactive buttons — send body + options as text.
          const rid = igRecipientId(lead);
          if (!rid) throw new Error('No Instagram recipient id on lead');
          const opts = (data.buttons || []).map((b, i) => `${i + 1}. ${b.title || b.id}`).join('\n');
          const result = await sendInstagramMessage(business, rid, [body, opts].filter(Boolean).join('\n\n'));
          if (!result.success) throw new Error(result.error || 'Instagram DM send failed');
        } else if (ctx.channel === 'facebook') {
          // Messenger quick-replies could be added later; send body + options as text for now.
          const rid = fbRecipientId(lead);
          if (!rid) throw new Error('No Facebook recipient id on lead');
          const opts = (data.buttons || []).map((b, i) => `${i + 1}. ${b.title || b.id}`).join('\n');
          const result = await sendMessengerMessage(business, rid, [body, opts].filter(Boolean).join('\n\n'));
          if (!result.success) throw new Error(result.error || 'Messenger DM send failed');
        } else {
      ...
      if (!isTest && lead && business) {
        if (ctx.channel === 'instagram') {
          ...
          const rows = (data.sections || [])
            ...
            .join('\n');
          const result = await sendInstagramMessage(business, rid, [body, rows].filter(Boolean).join('\n\n'));
          if (!result.success) throw new Error(result.error || 'Instagram DM send failed');
        } else if (ctx.channel === 'facebook') {
          const rid = fbRecipientId(lead);
          if (!rid) throw new Error('No Facebook recipient id on lead');
          const rows = (data.sections || [])
            .flatMap((s) => (s.rows || []).map((r) => `• ${r.title || r.id}`))
            .join('\n');
          const result = await sendMessengerMessage(business, rid, [body, rows].filter(Boolean).join('\n\n'));
          if (!result.success) throw new Error(result.error || 'Messenger DM send failed');
        } else {
    ...
    case 'action_update_contact': {
      if (lead?.contactId && data.fields && typeof data.fields === 'object') {
        const Contact = (await import('@/models/automation/Contact')).default;
        const contact = await Contact.findById(lead.contactId);
        if (contact) {
          for (const [k, v] of Object.entries(data.fields)) {
            contact[k] = renderTemplate(String(v), variables);
          }
          if (!ctx.execution.isTest) await contact.save();
        }
      }
    ...
    case 'action_assign': {
      if (lead && data.userId) {
        lead.assignedTo = data.userId;
        if (!ctx.execution.isTest) await lead.save();
      }
      return { message: data.userId ? `Assigned to ${data.userId}` : 'No assignee configured' };
    }
    case 'action_http':
    case 'action_webhook': {
      if (!isTest && data.url) {
...
export async function matchAndStartFlows({ business, lead, text = '', conversationId = null, event = 'incoming_message' }) {
  const triggerMap = {
    incoming_message: ['incoming_message', 'keyword'],
    keyword: ['keyword'],
    lead_created: ['lead_created'],
    contact_created: ['contact_created'],
    manual: ['manual'],
    webhook: ['webhook'],
    // Instagram — DMs and comments also evaluate keyword flows.
    instagram_dm: ['instagram_dm', 'keyword'],
    instagram_comment: ['instagram_comment', 'keyword'],
    // Facebook — Messenger DMs and Page comments also evaluate keyword flows.
    facebook_dm: ['facebook_dm', 'keyword'],
    facebook_comment: ['facebook_comment', 'keyword'],
  };
  const triggerTypes = triggerMap[event] || [event];

  // Channel isolation: an event must only start flows for its own channel.
  // Old flows predate the `channel` field, so treat missing/null as 'whatsapp'.
  ...
  const channel = eventChannelMap[event] || 'whatsapp';
  const channelFilter =
    channel === 'whatsapp'
      ? { $or: [{ channel: 'whatsapp' }, { channel: { $exists: false } }, { channel: null }] }
      : { channel };
