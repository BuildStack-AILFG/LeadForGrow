...
export async function ingestInboundEmail(businessId, {
  ...
  attachments = [],    // Cloudinary-uploaded files [{url, fileName, mimeType, size, isInline, contentId}].
  cc = [],             // [{name, address}] from parsed CC header — surfaced in the UI.
}) {
  ...
  const { message, conversation } = await recordChannelMessage({
    ...
    content: {
      ...
      // Sanitized HTML (dompurify) — safe to render via
      // dangerouslySetInnerHTML in the UI. Enables marketing / rich emails
      // to look normal instead of a stripped plain-text mess.
      html: html || '',
      participantId: from,
      participantEmail: from,
      participantName: fromName,
      // CC list from parsed headers — MessageBubble's EmailSenderHeader
      // renders it as a small dim "cc: name1, name2" line below the sender.
      cc: Array.isArray(cc) ? cc : [],
      // Persist every parsed attachment so the MessageBubble can render a
      // card per file. Even inline images from Gmail-style CID embeds land
      // here — user chose card-per-attachment over inline HTML rendering
      // so agents get a consistent UX across all file types.
...
/**
 * Ingest an OUTBOUND email discovered in the mailbox's Sent folder — i.e. a
 * message a human sent directly from Hostinger/Gmail webmail, OUTSIDE the CRM.
 * Mirrors ingestInboundEmail but keyed on the RECIPIENT (the customer) and
 * recorded as an outgoing message so the whole thread lives in one place.
 *
 * Dedup is critical: mail the CRM itself sent is recorded at send time with the
 * provider's RFC Message-ID, and its Sent copy carries the same id — so we skip
 * anything whose messageId we already have (belt-and-braces on top of
 * recordChannelMessage's own idempotency guard).
 */
export async function ingestOutboundEmail(businessId, {
  to,
  toName,
  subject,
  body,
  ...
  // Already recorded (the CRM sent it, or a previous overlapping sync saw it).
  if (externalMessageId) {
    const existing = await Message.findOne({ businessId, messageId: externalMessageId });
  ...
  // Threading — fold this sent mail into the existing thread when it's a reply.
  let parentMessage = null;
  if (inReplyTo) parentMessage = await Message.findOne({ businessId, messageId: inReplyTo });
  if (!parentMessage && references.length) {
    for (let i = references.length - 1; i >= 0; i--) {
      const ref = references[i];
      if (!ref) continue;
      const m = await Message.findOne({ businessId, messageId: ref });
      if (m) { parentMessage = m; break; }
    }
  }

  let thread = null;
  if (parentMessage?.emailThreadId) thread = await EmailThread.findById(parentMessage.emailThreadId);
  if (!thread && threadId) thread = await EmailThread.findOne({ businessId, externalThreadId: threadId });
  if (!thread) {
    thread = await EmailThread.create({
      businessId,
      emailAccountId: emailAccountId || null,
      leadId: matched.lead._id,
      subject,
      participants: [{ email: to, name: toName }],
      externalThreadId: threadId || externalMessageId,
      lastMessageAt: timestamp,
      messageCount: 1,
    });
  ...
  const { message, conversation } = await recordChannelMessage({
    ...
    companyId: matched.company?._id,
    dealId: matched.deal?._id,
    messageId: externalMessageId || `email_out_${Date.now()}`,
    direction: 'outgoing',
    type: 'email',
    content: {
      body: body || html?.replace(/<[^>]+>/g, '') || '',
    ...
    inReplyTo,
    references,
    // A human sent this by hand from webmail — provenance 'user', not automation.
    origin: 'user',
    folder: 'sent',
  });
...
export default { sendChannelEmail, ingestInboundEmail, ingestOutboundEmail };
