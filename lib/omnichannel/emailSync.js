...
import { ingestInboundEmail, ingestOutboundEmail } from '@/lib/omnichannel/emailService';
import { pickBulkHeaders } from '@/lib/omnichannel/responseTracking';
...
async function parseEmailAndAttachments(rawSource, { businessId, messageId }) {
  ...
  // List-Unsubscribe / Precedence / Auto-Submitted ... so newsletters and
  // notifications aren't counted as customers waiting for a reply.
  const bulkHeaders = pickBulkHeaders(parsed.headerLines);

  // Drop embedded images before we ever upload them. Signature logos, social
  // icons and tracking pixels ride along as inline images (marked inline, or
  // referenced by a Content-ID, or flagged `related` by mailparser). Surfacing
  // them as download cards clutters every email with a "logo.jpg 2 KB" card.
  // Genuine attached files (disposition 'attachment', no CID) are kept.
  const isEmbeddedImage = (att) => {
    const isImage = (att.contentType || '').toLowerCase().startsWith('image/');
    const embedded = att.contentDisposition === 'inline' || !!att.cid || att.related === true;
    return isImage && embedded;
  };
  ...
  const rawAttachments = (parsed.attachments || [])
    .filter((att) => !isEmbeddedImage(att))
  ...
  return { plainBody, htmlBody, inReplyTo, references, attachments, cc, bulkHeaders };
}
...
/**
 * Locate the Sent mailbox. Providers name it differently (Hostinger:
 * "INBOX.Sent", Gmail: "[Gmail]/Sent Mail", Outlook: "Sent Items"), so we
 * prefer the IMAP special-use \Sent flag and fall back to common names.
 * Returns the mailbox path, or null when there's nothing to sync.
 */
async function findSentMailbox(client) {
  ...
  const byName = boxes.find((b) => /(^|[./])sent( items| mail)?$/i.test(b.path || ''));
  return byName?.path || null;
}

/**
 * Read the Sent folder since `since` and record each message as an outgoing
 * CRM message. Returns how many NEW (non-deduped) messages were ingested.
 * Runs on its own mailbox lock AFTER the INBOX lock is released (imapflow
 * allows only one lock per connection at a time).
 */
async function syncSentFolder(client, account, since) {
  const sentPath = await findSentMailbox(client);
  if (!sentPath) return 0;
  ...
  try {
    for await (const msg of client.fetch(
      ...
      // Skip self-addressed mail (notes-to-self, bounce copies) — it would
      // create a lead for our own address.
      if (toEmail.toLowerCase() === (account.email || '').toLowerCase()) continue;

      const externalMessageId =
        msg.envelope?.messageId || `imap_${account._id}_sent_${msg.uid}`;

      const { plainBody, htmlBody, inReplyTo, references, attachments, cc } =
        await parseEmailAndAttachments(msg.source, {
      ...
      try {
        const res = await ingestOutboundEmail(account.businessId, {
          to: toEmail,
          toName: to?.name,
        ...
        if (res?.message && !res.deduped) count += 1;
      } catch (perMsgError) {
        console.error(
          `[emailSync] account=${account._id} sent uid=${msg.uid} failed:`,
          perMsgError.message
        );
      }
    }
...
export async function syncEmailAccount(account) {
  ...
  try {
    ...
    // Lifted above the mailbox locks so the Sent-folder pass reuses it.
    ...
    // ---- Sent-folder pass ----------------------------------------------
    // Capture mail a human sent DIRECTLY from webmail (Hostinger/Gmail),
    // outside the CRM, so the full thread lives in one place. Recorded as
    // outgoing, keyed on the recipient, deduped against CRM-sent copies by
    // Message-ID. Best-effort: any failure here must not fail the INBOX sync
    // that already succeeded.
    try {
      synced += await syncSentFolder(client, account, since);
    } catch (sentErr) {
      console.warn(`[emailSync] account=${account._id} Sent-folder sync skipped:`, sentErr.message);
    }
