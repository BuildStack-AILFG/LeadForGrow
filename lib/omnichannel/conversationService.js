...
import { normalizeTimestamp } from '@/lib/omnichannel/timestamps';
...
import { isCustomerInbound, responseTrackingOps, firstReplyUpdate } from '@/lib/omnichannel/responseTracking';
...
function previewFromMessage({ type, body, subject, isInternal }) {
  ...
  // For emails, prefer the actual body text over the subject — the subject
  // by itself is redundant with the sticky subject bar above the thread
  // AND with the ConversationItem's title. Body preview is what agents
  // actually scan the sidebar for. Collapse whitespace + strip any stray
  // HTML that leaked past the sync pipeline.
  if (type === 'email') {
    const cleanBody = (body || '')
      ...
      .trim();
    if (cleanBody) return cleanBody.substring(0, 120);
    return (subject || 'New email').substring(0, 100);
  }
...
/** Reopen a conversation that was marked Done. Spam / archived stay as they are. */
export async function reopenIfClosed(conversationId) {
  if (!conversationId) return;
  await Conversation.updateOne(
    { _id: conversationId, status: 'closed' },
...
export async function upsertConversation(businessId, {
  ...
  trackingOps = {}, // { $min?, $unset? } from responseTrackingOps — first-response fields.
}) {
  const timestamp = normalizeTimestamp(rawTimestamp); // seconds mistaken for milliseconds would store 1970
  const update = {
    leadId,
    contactId,
    companyId,
  ...
  // A customer writing again reopens a conversation someone marked Done (closed): otherwise it would stay out of
  // the "Needs reply" queue and the customer would be invisible.
  if (direction === 'incoming' && incrementUnread) await reopenIfClosed(conversation._id);
...
export async function recordChannelMessage({
  ...
  emailThreadId,
  rawMetadata,
  // Email mailbox folder ('inbox' | 'sent' | 'drafts' | 'trash' | 'starred').
  // Optional — when omitted the field stays unset (inbound stays null, which
  // the inbox query already treats as inbox). Outbound email callers pass
  // 'sent' so the message lands in the Sent tab.
  folder,
  ...
  // First-response tracking (see lib/omnichannel/responseTracking.js).
  const trackingOps = responseTrackingOps({
    direction,
    origin,
    status,
    isInternal,
    timestamp,
    customerInbound: direction === 'incoming' && isCustomerInbound({
      channel,
      participantEmail: content.participantEmail || conversation?.participantEmail,
      headers,
      isInternal,
    }),
  });
  ...
  if (!conversation) {
    ...
    if (direction === 'incoming' && !isInternal) await reopenIfClosed(conversation._id);
  }

  // One write per conversation lifetime: stamp the first human / automated reply.
  const firstReply = firstReplyUpdate({ direction, origin, status, isInternal, timestamp, conversation });
  if (firstReply) {
    try {
      // updatePipeline: firstResponseMs is computed from the stored firstInboundAt (Mongoose 9 opt-in).
      await Conversation.updateOne(firstReply.filter, firstReply.update, { updatePipeline: true });
    } catch (err) {
      console.error('[conversationService] first-response tracking failed:', err.message);
    }
  }
  ...
  if (!isInternal) {
    ...
    // Who wrote: the browser toast used to say "from a customer" because the event carried no name and the list on screen
    // rarely holds that conversation. The name (and a short preview) now travels with the event.
    const lead = direction === 'incoming' && leadId ? await Lead.findById(leadId).select('name').lean() : null;
    const senderName = direction === 'incoming'
      ? (lead?.name || conversation.participantName || conversation.participantEmail || conversation.participantPhone || null)
      : null;
    await emitChatMessage(businessId, {
      ...
      ...(direction === 'incoming' && preview ? { preview: String(preview).slice(0, 120) } : {}),
    });
...
export async function assignConversation(conversationId, businessId, { toUserId, fromUserId, assignedBy, reason }) {
  ...
  // The Leads page reads the LEAD's assignee. Fill it when nobody owns the lead yet, so a conversation claimed from the
  // inbox does not keep showing "Unassigned" there; an existing lead owner is never overwritten.
  if (conv.leadId) {
    await Lead.updateOne(
      { _id: conv.leadId, businessId, $or: [{ assignedTo: null }, { assignedTo: { $exists: false } }] },
