...
export async function matchCustomer(businessId, {
  ...
  // 3b. Facebook (Messenger PSID / commenter id)
  if (!result.lead && facebookId) {
    const leadByFb = await Lead.findOne({
      businessId,
      archived: { $ne: true },
      $or: [
        { 'metadata.facebookId': facebookId },
        { 'customFields.facebookId': facebookId },
      ],
    });
    if (leadByFb) result.lead = leadByFb;
  }
  ...
  if (!result.lead && createIfMissing) {
    const sourceMap = { whatsapp: 'whatsapp', instagram: 'instagram', facebook: 'facebook', email: 'website' };
    const leadData = {
      businessId,
      name: name || instagramUsername || facebookName || emailNorm?.split('@')[0] || 'New Contact',
    ...
    if (facebookId) {
      leadData.metadata = { ...(leadData.metadata || {}), facebookId, facebookName };
    }
    ...
    // Real-time: notify the workspace (sound + toast) that a new lead just
    // landed. Centralized here since WhatsApp/Instagram/Email inbound
    // messages all create leads through this one function — fixes the gap
    // where only WhatsApp used to fire this via its own caller.
    try {
      ...
      console.warn('[matchCustomer] emitLeadUpdated failed:', err.message);
    }
  }
