...
export default function FollowUpTasks({ tasks = [] }) {
  ...
  return (
    <ChartCard
      ...
      action={
        <Link href="/automation/tasks" className="text-xs font-medium text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300 flex items-center gap-1">
    ...
    >
      {sorted.length === 0 ? (
        <div className="py-8 text-center">
          ...
          <Link href="/automation/tasks" className="text-xs text-teal-600 dark:text-teal-400 hover:underline mt-1 inline-block">
            Create a follow-up
          </Link>
        </div>
      ) : (
