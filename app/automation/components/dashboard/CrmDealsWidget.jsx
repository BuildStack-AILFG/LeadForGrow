  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/automation/deals" className="text-xs text-emerald-600 dark:text-emerald-400 hover:underline">View deals</Link>
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-4">
        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
          <p className="text-xs text-slate-500 dark:text-slate-400">Today&apos;s Leads</p>
          <p className="text-lg font-bold text-slate-900 dark:text-white">{todayLeads || 0}</p>
        </div>
        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
          <p className="text-xs text-slate-500 dark:text-slate-400">Open Deals</p>
        ...
        <div className="p-3 bg-emerald-50/60 dark:bg-emerald-950/20 rounded-lg">
          <p className="text-xs text-slate-500 dark:text-slate-400">Revenue Won</p>
        ...
        <div className="p-3 bg-teal-50/60 dark:bg-teal-950/20 rounded-lg">
          <p className="text-xs text-slate-500 dark:text-slate-400">In Pipeline</p>
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg col-span-2 sm:col-span-1">
          <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1"><TrendingUp className="w-3 h-3" /> Lead Conversion</p>
          <p className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{conversionRate || 0}%</p>
        </div>
      </div>

      {pipeline?.length > 0 && (
        <div className="mb-4">
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-2">Deal pipeline</p>
      ...
      {recentWins?.length > 0 && (
        <div>
          <p className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-2 flex items-center gap-1"><Trophy className="w-3 h-3" /> Recent Wins</p>
          <div className="space-y-2">
            {recentWins.slice(0, 3).map((d) => (
              <Link key={d._id} href={`/automation/deals/${d._id}`} className="flex justify-between text-sm hover:text-emerald-600 dark:hover:text-emerald-400">
                <span className="truncate">{d.title}</span>
                <span className="font-medium text-emerald-600 dark:text-emerald-400 ml-2">{formatValue(d.amount, d.currency)}</span>
              </Link>
            ))}
          </div>
        </div>
