...
function InsightTile({ icon: Icon, label, value, sub, color, href }) {
  ...
  <div className={`p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-violet-300 dark:hover:border-violet-700 transition-colors ${href ? 'cursor-pointer' : ''}`}>
      ...
      {sub && <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{sub}</p>}
    </div>
  );
  ...
  return (
    <DashboardCard padding="p-5">
      <div className="flex items-center justify-between gap-2 mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Powered by Grovia</p>
          </div>
        </div>
        <Link href="/automation/ai/knowledge" className="text-xs text-violet-600 dark:text-violet-400 hover:underline flex items-center gap-0.5">
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 py-4">Insights unavailable</p>
      ) : (
        <>
          ...
          {data.nextBestActions?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Next Best Actions</p>
