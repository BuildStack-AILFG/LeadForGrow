...
export default function ContactsImportModal({ open, onClose, onImported }) {
  ...
  const importRow = async (row) => {
    const firstName = findValue(row, ['first name', 'firstname', 'name']);
    if (!firstName) return { ok: false, label: '(missing first name)', error: 'First name is required' };

    const lastName = findValue(row, ['last name', 'lastname']);
    const email = findValue(row, ['email', 'email address']);
    const phone = findValue(row, ['phone', 'phone number', 'mobile']);
    const label = [firstName, lastName].filter(Boolean).join(' ');

    const payload = {
      firstName,
      lastName: lastName || undefined,
      jobTitle: findValue(row, ['job title', 'title']) || undefined,
      type: findValue(row, ['type']) || undefined,
      emails: email ? [{ address: email, primary: true }] : [],
      phones: phone ? [{ number: phone, primary: true }] : [],
    };

    try {
      const res = await authFetch('/api/automation/contacts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      const data = await res.json();
      if (data.success) return { ok: true, label };
      return { ok: false, label, error: data.error || 'Failed' };
    } catch (err) {
      return { ok: false, label, error: err.message || 'Network error' };
    }
  };
  ...
  const startImport = async () => {
    ...
    setStatus('done');
    onImported?.();
    toast.success(`Imported ${success} of ${rows.length} contact${rows.length === 1 ? '' : 's'}`);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded shadow-xl border border-[#E5E7EB] max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#F2F4F7]">
          <h3 className="text-[15px] font-semibold text-[#101828]">Import Contacts from CSV</h3>
          {status !== 'processing' && (
          ...
          {status === 'idle' && (
            <>
              <p className="text-[13px] text-[#667085]">
                Upload a CSV with a <span className="font-medium">First Name</span> column (required), plus any of
                Last Name, Email, Phone, Job Title, Type. Column names are matched automatically, case-insensitive.
              </p>
              <input ref={fileInputRef} type="file" accept=".csv" className="hidden" onChange={handleFile} id="contacts-csv-input" />
              <label
