          ...
          >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h2 className="text-[17px] font-semibold text-[#101828] dark:text-slate-100 truncate">{name}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5">
              {loading ? (
                ...
                <p className="text-[13px] text-[#667085] dark:text-slate-300 text-center py-12">Contact not found</p>
              )}
            </div>
          </motion.aside>
        </>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[#E5E7EB] dark:border-slate-700 p-4 space-y-3">
        <h3 className="text-[12px] font-semibold uppercase tracking-wide text-[#98A2B3] dark:text-slate-400">Contact Details</h3>
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[13px]">
          {email && (
            <div>
              ...
              <dd className="flex items-center gap-1 text-[#344054] dark:text-slate-200"><Mail className="w-3.5 h-3.5" />{email}</dd>
            </div>
          )}
          {phone && (
            <div>
              <dt className="text-[#98A2B3] dark:text-slate-400 text-[11px] mb-0.5">Phone</dt>
              <dd className="flex items-center gap-1 text-[#344054] dark:text-slate-200"><Phone className="w-3.5 h-3.5" />{phone}</dd>
            </div>
          )}
          {contact.jobTitle && (
            <div>
              <dt className="text-[#98A2B3] dark:text-slate-400 text-[11px] mb-0.5">Job Title</dt>
              <dd className="text-[#344054] dark:text-slate-200">{contact.jobTitle}</dd>
            </div>
          )}
          {contact.department && (
            <div>
              <dt className="text-[#98A2B3] dark:text-slate-400 text-[11px] mb-0.5">Department</dt>
              <dd className="text-[#344054] dark:text-slate-200">{contact.department}</dd>
            </div>
          )}
          {company?.name && (
            <div>
              ...
              <dd className="flex items-center gap-1 text-[#344054] dark:text-slate-200">
                ...
                <Link href={`/automation/companies/${company._id || company}`} className="text-[#059669] dark:text-emerald-400 hover:underline">
                  {company.name}
                </Link>
              </dd>
            </div>
        ...
        {contact.notes && typeof contact.notes === 'string' && (
          <p className="text-[13px] text-[#475467] dark:text-slate-300 leading-relaxed pt-2 border-t border-[#F2F4F7] dark:border-slate-700">{contact.notes}</p>
        )}
      </div>
    </div>
  );
        ...
        <tbody>
          {deals.map((d) => (
            <tr key={d._id} className="border-b border-[#F2F4F7] dark:border-slate-700 hover:bg-[#FAFBFC] dark:hover:bg-slate-800">
              ...
              <td className="py-3 px-3 tabular-nums text-[#101828] dark:text-slate-100">{formatCurrency(d.amount, d.currency)}</td>
