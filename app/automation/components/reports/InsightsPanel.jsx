...
export default function InsightsPanel({ insights = [] }) {
  return (
    <ChartCard title="Intelligence Insights" subtitle="Data-driven recommendations">
      {insights.length === 0 ? (
        <p className="text-sm text-slate-500 dark:text-slate-400 py-4">Insights will appear as your pipeline grows.</p>
      ) : (
        <ul className="space-y-2">
          {insights.map((item) => (
            ...
            >
              <Sparkles className="w-3.5 h-3.5 text-teal-600 dark:text-teal-400 flex-shrink-0 mt-0.5" />
