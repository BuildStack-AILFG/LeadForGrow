      ...
      <div className="fixed inset-4 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-3xl sm:max-h-[90vh] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl z-50 flex flex-col border border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">{form.name}</p>
          </div>
          <button type="button" onClick={onClose} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"><X className="w-5 h-5" /></button>
        </div>
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`px-3 py-2 text-xs font-medium rounded-t-lg transition-colors ${
                tab === t.id ? 'text-teal-600 dark:text-teal-400 border-b-2 border-teal-600' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              {t.label}
            </button>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          ...
          {tab === 'react' && (
            <>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-3">Use the HTML widget inside a React component via dangerouslySetInnerHTML or a portal.</p>
            </>
          )}
            ...
            <div className="bg-teal-50 dark:bg-teal-950/30 border border-teal-100 dark:border-teal-900 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 mb-3">Share in ads, WhatsApp, or email — no website needed.</p>
                  <div className="flex gap-2">
                    <code className="flex-1 text-xs bg-white dark:bg-slate-900 p-2 rounded-lg border break-all">{snippets.hostedLink}</code>
                    <button type="button" onClick={() => copy(snippets.hostedLink)} className="p-2 text-teal-600 dark:text-teal-400"><Copy className="w-4 h-4" /></button>
                    <a href={snippets.hostedLink} target="_blank" rel="noopener noreferrer" className="p-2 text-teal-600 dark:text-teal-400"><ExternalLink className="w-4 h-4" /></a>
                  </div>
                </div>
              </div>
            </div>
