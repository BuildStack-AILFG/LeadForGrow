      ...
      <div className="space-y-1.5">
        {PLACEHOLDERS.map((p) => (
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 text-left rounded bg-slate-50 dark:bg-slate-800/50 hover:bg-[#1D4B3E]/10 dark:hover:bg-teal-950/30 transition-colors group"
          >
            <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{p.label}</span>
            <code className="text-[10px] text-slate-400 group-hover:text-[#1D4B3E] font-mono">{p.value}</code>
          </button>
        ))}
      </div>
    </div>
