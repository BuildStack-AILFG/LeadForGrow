      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">{isReadOnly ? 'Meta templates are read-only' : 'Customize your message'}</p>
          </div>
          <button type="button" onClick={onClose} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {!isReadOnly && (
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Template name</label>
              <input
                value={draft.name}
          ...
          {!isReadOnly && (
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Channel</label>
          ...
          {draft.channel === 'email' && (
            <div>
              <label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Subject line</label>
              <input
                value={draft.subject || ''}
        ...
        <div className="px-6 py-4 border-t border-slate-100 dark:border-slate-800 flex gap-2">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded">
            Cancel
          </button>
          {!isReadOnly && (
...
const inputClass = 'w-full px-4 py-2.5 text-sm bg-slate-50 dark:bg-slate-800/80 border-0 rounded focus:outline-none focus:ring-2 focus:ring-brand/25 disabled:opacity-60';
