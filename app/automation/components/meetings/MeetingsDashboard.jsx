      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Link
            ...
            className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors"
          >
            Analytics
          </Link>
          <button
        ...
        <DashboardCard className="xl:col-span-2" padding="p-0">
          <div className="px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">Guest bookings from your /book links</p>
            </div>
            <Link href="/automation/meetings/team" className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1">
            ...
            {(dashboard?.upcomingBookings || []).length === 0 ? (
              <div className="p-6 sm:p-8">
                ...
                {(dashboard?.bookingLinks || []).length > 0 ? (
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 text-center">Publish a booking link to start accepting meetings.</p>
                )}
              </div>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <DashboardCard padding="p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/automation/meetings/templates" className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">
              Templates
            </Link>
          </div>
          <div className="space-y-2">
            ...
            {!dashboard?.bookingLinks?.length && (
              <p className="text-sm text-slate-500 dark:text-slate-400 py-4 text-center">Publish a meeting type to get your booking link.</p>
            )}
          </div>
        </DashboardCard>
            ...
            {(dashboard?.recentBookings || []).map((b) => (
              <div key={b._id} className="text-sm py-2 border-b border-slate-50 dark:border-slate-800 last:border-0">
                ...
                {b.whatsappConfirmationSent && (
                  <span className="ml-2 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-1.5 py-0.5 rounded">
                    WA sent
                  </span>
                )}
              </div>
