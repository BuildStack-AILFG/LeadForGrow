              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-slate-900 dark:text-slate-100">
                    {session.device}
                    {session.current && <span className="ml-2 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 px-1.5 py-0.5 rounded">Current</span>}
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{session.location} · {session.lastActive}</p>
                </div>
              </div>
              {!session.current && (
                <button type="button" className="text-xs font-medium text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 flex items-center gap-1">
            ...
            <div key={token.id} className="flex items-center justify-between gap-3 py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">{token.prefix} · Created {token.created}</p>
                </div>
              </div>
              <span className="text-[10px] text-slate-400">Used {token.lastUsed}</span>
            </div>
          ))}
          <button type="button" className="text-xs font-medium text-teal-600 dark:text-teal-400 hover:text-teal-700 dark:hover:text-teal-300">+ Generate new token</button>
        </div>
      </SettingsCard>
          ...
          {security.auditLogs.map((log) => (
            <div key={log.id} className="flex items-start gap-3 py-2 text-xs border-b border-slate-100 dark:border-slate-800 last:border-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-500 dark:text-slate-400">{log.user} · {log.time} · {log.ip}</p>
              </div>
            </div>
          ))}
        </div>
