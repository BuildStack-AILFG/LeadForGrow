...
export default function QualityRatingBanner({ audienceCount, className = '' }) {
  ...
  if (loading) {
    return (
      <div className={`rounded-xl border border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2 ${className}`}>
  ...
  if (error) {
    return (
      <div className={`rounded-xl border border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-500 dark:text-slate-400 ${className}`}>
