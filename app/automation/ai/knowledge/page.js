...
const STATUS_COLORS = {
  ready: 'text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-950/30',
    ...
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700"
        >
      ...
      {searchResults && (
        <div className="p-4 rounded-xl bg-teal-50 dark:bg-teal-950/20 border border-teal-200 dark:border-teal-800 space-y-2">
          <p className="text-xs font-semibold text-teal-700 dark:text-teal-300">{searchResults.length} results</p>
          {searchResults.map((r, i) => (
            <div key={i} className="text-xs text-slate-600 dark:text-slate-400 p-2 bg-white/60 dark:bg-slate-900/60 rounded-lg">
              <span className="font-medium text-teal-600 dark:text-teal-400">{r.sourceName}</span>
      ...
      {showForm && (
        <form onSubmit={createSource} className="p-5 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-3">
          ...
          {(form.type === 'pdf' || form.type === 'docx') && (
            <div>
              <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl py-6 cursor-pointer hover:border-teal-400 transition-colors">
              ...
              {file && <p className="text-[11px] text-teal-600 dark:text-teal-400 mt-1.5">✓ {file.name} uploaded — click Add &amp; Index to process</p>}
            </div>
          )}
          ...
          {form.type === 'faq' && (
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => setFaqs([...faqs, { question: '', answer: '' }])} className="inline-flex items-center gap-1 text-xs font-medium text-teal-600 dark:text-teal-400 hover:underline">
          ...
          {form.type === 'catalog' && (
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => setCatalog([...catalog, { name: '', price: '', sku: '', description: '' }])} className="inline-flex items-center gap-1 text-xs font-medium text-teal-600 dark:text-teal-400 hover:underline">
          ...
          <div className="flex gap-2 pt-1">
            <button type="submit" disabled={submitting || uploading} className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-sm font-medium disabled:opacity-50 inline-flex items-center gap-1.5">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-500 dark:text-slate-400">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700"
          >
